...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        ...
        slivers: [
          SliverToBoxAdapter(
            child: _buildInsightCard(tx, colors),
          ),
          SliverToBoxAdapter(
  ...
  Widget _buildInsightCard(TransactionProvider tx, ThemeColors colors) {
    final isExpense = _insightType == 'EXPENSE';
    final accentColor = isExpense ? AppColors.expense : AppColors.income;

    // Fallback: Calculate stats locally from current transactions
    Map<String, double> data = {};
    
    // Check if we have API stats first
    final statsKey = isExpense ? 'expenses_by_category' : 'incomes_by_category';
    ...
    // Local calculation logic (Fallback/Reactive)
    if (data.isEmpty) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            child: hasData
                    ...
                    PieChartData(
                      ...
                      sections: data.entries.map((e) {
                        ...
                        final colorsList = isExpense 
                          ? [AppColors.expense, AppColors.accent, const Color(0xFFFACC15), const Color(0xFFFB923C)]
                          : [AppColors.income, const Color(0xFF3B82F6), const Color(0xFF8B5CF6), const Color(0xFFEC4899)];
                ...
                : Center(
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.pie_chart_outline_rounded, color: colors.textMuted, size: 40),
                        const SizedBox(height: 8),
                        Text(
          ...
          if (hasData) const SizedBox(height: 32),
          if (hasData)
            Wrap(
              spacing: 16,
              runSpacing: 10,
              children: data.entries.take(4).map((e) {
                ...
                return Row(
                  ...
                  children: [
                    ...
                    Text(e.key, style: TextStyle(color: colors.textSecondary, fontSize: 12, fontWeight: FontWeight.w500)),
                  ],
                );
              }).toList(),
            ),
